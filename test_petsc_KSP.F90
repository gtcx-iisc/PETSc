...
implicit none
  ...
  do i=rstart,rend-1
   ...
   if (i==0) then
      call MatSetValue(A,i,m-1,two,INSERT_VALUES,ierr);  CHKERRQ(ierr)
   else if (i==m-1) then
      call MatSetValue(A,i,0,two,INSERT_VALUES,ierr);  CHKERRQ(ierr)
   else
      call MatSetValue(A,i,i+1,two,INSERT_VALUES,ierr);  CHKERRQ(ierr)
      call MatSetValue(A,i,i-1,two,INSERT_VALUES,ierr);  CHKERRQ(ierr)
   end if
